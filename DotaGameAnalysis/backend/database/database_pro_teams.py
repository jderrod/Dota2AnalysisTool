...
class ProLeague(Base):
    ...
    # Relationships
    matches = relationship("ProMatch", back_populates="league")
...
class ProTeam(Base):
    ...
    players = relationship("ProPlayer", back_populates="team")
    
    def __repr__(self):
        return f"<ProTeam(id={self.id}, name='{self.name}')>"
...
class ProMatch(Base):
    ...
    # League information
    league_id = Column(Integer, ForeignKey('pro_leagues.league_id'))
    league = relationship("ProLeague", back_populates="matches")  # back_populates must match League.matches
    
    series_id = Column(Integer, nullable=True)
    series_type = Column(Integer, nullable=True)
    
    # Team information
    radiant_team_id = Column(Integer, ForeignKey('pro_teams.team_id'), nullable=True)
    dire_team_id = Column(Integer, ForeignKey('pro_teams.team_id'), nullable=True)
...
class ProPlayer(Base):
    ...
    id = Column(Integer, primary_key=True)
    match_id = Column(Integer, ForeignKey('pro_matches.match_id'), nullable=True)
    account_id = Column(Integer, unique=True)
    name = Column(String(255))
    personaname = Column(String(255), nullable=True)
    country_code = Column(String(10), nullable=True)
    team_id = Column(Integer, ForeignKey('pro_teams.team_id'), nullable=True)
    
    # Relationships
    match_players = relationship("ProMatchPlayer", back_populates="player")
    
    # Relationship to Team
    team = relationship("ProTeam", back_populates="players")
...
class ProMatchPlayer(Base):
    ...
    """
    __tablename__ = 'pro_match_player_metrics'
    __table_args__ = (
        UniqueConstraint('match_id', 'account_id', 'hero_id', name='uix_match_player'),
    )

    id = Column(Integer, primary_key=True)
    match_id = Column(Integer, ForeignKey('pro_matches.match_id'))
    account_id = Column(Integer, ForeignKey('pro_players.account_id'))
    hero_id = Column(Integer, ForeignKey('pro_heroes.hero_id'))
    ...
    player = relationship("ProPlayer", back_populates="match_players")


    def __repr__(self):
        return (f"<ProMatchPlayer(match_id={self.match_id}, account_id={self.account_id}, "
                f"hero_id={self.hero_id})>")


class ProDraftTiming(Base):
    __tablename__ = 'pro_draft_timings'
    ...
    def __repr__(self):
        return (f"<ProDraftTiming(order={self.order}, pick={self.pick}, "
...
class ProTeamFight(Base):
    """Table for storing teamfight-level information in a match."""
    __tablename__ = 'pro_teamfights'
    
    id = Column(Integer, primary_key=True)
    match_id = Column(Integer, ForeignKey('pro_matches.match_id'))  # Optionally add a ForeignKey('matches.match_id') later
    ...
    players = relationship("ProTeamFightPlayer", back_populates="teamfight")
    
    def __repr__(self):
        return (f"<ProTeamFight(match_id={self.match_id}, start={self.start}, end={self.end}, "
                f"last_death={self.last_death}, deaths={self.deaths})>")


class ProTeamFightPlayer(Base):
    """Table for storing per-player statistics within a teamfight."""
    __tablename__ = 'pro_teamfight_players'
    
    id = Column(Integer, primary_key=True)
    match_id = Column(Integer, ForeignKey('pro_matches.match_id'), nullable=True)
    teamfight_id = Column(Integer, ForeignKey('pro_teamfights.id'))  # Link to the corresponding teamfight
    ...
    teamfight = relationship("ProTeamFight", back_populates="players")
    
    def __repr__(self):
        return (f"<ProTeamFightPlayer(teamfight_id={self.teamfight_id}, "
...
class ProObjective(Base):
    __tablename__ = 'pro_objectives'
    
    id = Column(Integer, primary_key=True)
    match_id = Column(Integer, ForeignKey('pro_matches.match_id'))  # Optionally add ForeignKey('matches.match_id') later
    ...
    def __repr__(self):
        return f"<ProObjective(time={self.time}, type='{self.type}', key='{self.key}')>"


class ProChatWheel(Base):
    __tablename__ = 'pro_chatwheel'
    ...
    def __repr__(self):
        return (f"<ProChatWheel(match_id={self.match_id}, time={self.time}, "
                f"type='{self.type}', key='{self.key}', slot={self.slot}, "
                f"player_slot={self.player_slot})>")


class ProTimevsStats(Base):
    __tablename__ = 'pro_timevsstats'
    id = Column(Integer, primary_key=True)
    match_id = Column(Integer, ForeignKey('pro_matches.match_id'))
    player_id = Column(Integer, ForeignKey('pro_players.id'))
    ...
    def __repr__(self):
        return (f"<ProTimevsStats(match_id={self.match_id}, player_slot={self.player_slot}, "
...
class DotaDatabase:
    ...
    def update_all_tables(self, days_to_look_back=7):
        ...
        teams = session.query(ProTeam).all()
        session.close()
        ...
        for team in teams:
            ...
            try:
                ...
                session = self.Session()
                existing_match_ids = {
                    row[0] for row in session.query(ProMatch.match_id).filter(
                        (ProMatch.radiant_team_id == team.team_id) | 
                        (ProMatch.dire_team_id == team.team_id)
                    ).all()
                }
                session.close()
    ...
    def search_matches(self, criteria=None, limit=20, order_by_recent=True):
        ...
        if 'team_id' in criteria:
            ...
            query = query.filter(or_(ProMatch.radiant_team_id == team_id, ProMatch.dire_team_id == team_id))
        elif 'team_name' in criteria:
            team_name = criteria['team_name']
            # Find team IDs matching the name
            team_query = session.query(ProTeam.team_id).filter(ProTeam.name.ilike(f'%{team_name}%'))
            team_ids = [t[0] for t in team_query.all()]
            if team_ids:
                team_filter = or_(*[
                    or_(ProMatch.radiant_team_id == tid, ProMatch.dire_team_id == tid) 
                    for tid in team_ids
                ])
        ...
        if 'player_id' in criteria or 'player_name' in criteria:
            ...
            query = query.join(ProMatchPlayer, ProMatchPlayer.match_id == ProMatch.match_id)
            query = query.join(ProPlayer, ProPlayer.account_id == ProMatchPlayer.account_id)
            
            if 'player_id' in criteria:
                query = query.filter(ProPlayer.account_id == criteria['player_id'])
            elif 'player_name' in criteria:
                player_name = criteria['player_name']
                query = query.filter(or_(
                    ProPlayer.name.ilike(f'%{player_name}%'),
                    ProPlayer.personaname.ilike(f'%{player_name}%')
                ))
        ...
        if 'hero_id' in criteria or 'hero_name' in criteria:
            ...
            query = query.join(ProHero, ProHero.hero_id == ProMatchPlayer.hero_id)
            
            if 'hero_id' in criteria:
                query = query.filter(ProHero.hero_id == criteria['hero_id'])
            elif 'hero_name' in criteria:
                hero_name = criteria['hero_name']
                query = query.filter(or_(
                    ProHero.name.ilike(f'%{hero_name}%'),
                    ProHero.localized_name.ilike(f'%{hero_name}%')
                ))
        ...
        if 'min_date' in criteria:
            ...
            query = query.filter(ProMatch.start_time >= min_date)
        ...
        if 'max_date' in criteria:
            ...
            query = query.filter(ProMatch.start_time <= max_date)
        
        # Filter by duration
        if 'min_duration' in criteria:
            query = query.filter(ProMatch.duration >= criteria['min_duration'])
            
        if 'max_duration' in criteria:
            query = query.filter(ProMatch.duration <= criteria['max_duration'])
            
        # Filter by league
        if 'league_id' in criteria:
            query = query.filter(ProMatch.league_id == criteria['league_id'])
        
        # Order by most recent matches first if requested
        if order_by_recent:
            query = query.order_by(desc(ProMatch.start_time))
    ...
    def get_match_with_details(self, match_id):
        ...
        # Get the match
        match = session.query(ProMatch).filter(ProMatch.match_id == match_id).first()
        if not match:
            session.close()
            return None
        ...
        if match.radiant_team_id:
            radiant_team = session.query(ProTeam).filter(ProTeam.team_id == match.radiant_team_id).first()
        
        if match.dire_team_id:
            dire_team = session.query(ProTeam).filter(ProTeam.team_id == match.dire_team_id).first()
        
        # Get player details
        players = session.query(
            ProMatchPlayer, ProPlayer, ProHero
        ).join(
            ProPlayer, ProPlayer.account_id == ProMatchPlayer.account_id
        ).join(
            ProHero, ProHero.hero_id == ProMatchPlayer.hero_id
        ).filter(
            ProMatchPlayer.match_id == match_id
        ).all()
...
def populate_from_json(json_path):
    ...
    # 2. Upsert the Match
    existing_match = session.query(ProMatch).filter_by(match_id=match_id).one_or_none()
    if existing_match:
        ...
        existing_match.league_id = data.get("league_id", existing_match.league_id)
        existing_match.series_id = data.get("series_id", existing_match.series_id)
        existing_match.series_type = data.get("series_type", existing_match.series_type)
        existing_match.radiant_team_id = data.get("radiant_team_id", existing_match.radiant_team_id)
        existing_match.dire_team_id = data.get("dire_team_id", existing_match.dire_team_id)
        existing_match.game_version = data.get("version", existing_match.game_version)
        
        # Update gold advantages if available
        if data.get("radiant_gold_adv") and existing_match.radiant_gold_adv is None:
            existing_match.radiant_gold_adv = data.get("radiant_gold_adv")[-1] if data.get("radiant_gold_adv") else 0
        
        # Calculate dire gold advantage as negative of radiant (if needed)
        if data.get("radiant_gold_adv") and existing_match.dire_gold_adv is None:
            existing_match.dire_gold_adv = -1 * (data.get("radiant_gold_adv")[-1] if data.get("radiant_gold_adv") else 0)
        ...
        if start_time_unix:
            ...
            logger.warning(f"No start_time found in match {match_id} JSON. Using current time instead.")
        
        # Calculate dire gold advantage as negative of radiant
        dire_gold_adv = None
        if data.get("radiant_gold_adv"):
            radiant_gold_adv = data.get("radiant_gold_adv")[-1]
            dire_gold_adv = -1 * radiant_gold_adv
        else:
            radiant_gold_adv = 0
            dire_gold_adv = 0
        ...
        match_record = ProMatch(
            ...
            league_id=data.get("league_id"),  # Make sure to populate league_id
            series_id=data.get("series_id"),  # Make sure to populate series_id
            series_type=data.get("series_type"),  # Make sure to populate series_type
            radiant_team_id=data.get("radiant_team_id"),  # Make sure to populate radiant_team_id
            dire_team_id=data.get("dire_team_id"),  # Make sure to populate dire_team_id
    ...
    for p in data.get("players", []):
        ...
        # Upsert Player
        existing_player = session.query(ProPlayer).filter_by(account_id=account_id).one_or_none()
        if not existing_player:
            new_player = ProPlayer(
                match_id=match_id,
        ...
        # Upsert Hero
        existing_hero = session.query(ProHero).filter_by(hero_id=hero_id).one_or_none()
        ...
        existing_mp = session.query(ProMatchPlayer).filter_by(
            match_id=match_id,
...
if __name__ == "__main__":
    ...
    # Use raw SQL to drop all tables - this will work for old and new tables
    ...
    print("Creating new pro_ prefixed tables...")
    Base.metadata.create_all(engine)
    
    # Specify the root directory containing your teams
    root_directory_games = r"C:\Users\jtder\Dota2ProGames\DotaProMatchAnalysis\data\raw\teams_games"
    root_directory_data = r"C:\Users\jtder\Dota2ProGames\DotaProMatchAnalysis\data\raw\team_data"

    # Recursively walk through the directory
    for dirpath, dirnames, filenames in os.walk(root_directory_games):
        for filename in filenames:
            if filename.endswith(".json"):
                json_file = os.path.join(dirpath, filename)
                print(f"Processing {json_file}...")
                populate_from_json(json_file)
    print("Done processing team games JSON files.")
    
    for dirpath, dirnames, filenames in os.walk(root_directory_data):
        for filename in filenames:
            if filename.endswith(".json"):
                json_file = os.path.join(dirpath, filename)
                print(f"Processing {json_file}...")
                populate_team_from_json(json_file)
    ...
    print("Done processing team data JSON files.")
    
    # After rebuilding database, you can switch to this to only update new data
    """
